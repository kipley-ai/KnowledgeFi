...
export default function DataSource() {
    ...
    return (
        <div className="flex flex-col sm:px-6 lg:px-8 py-8 bg-[#292D32]">
            ...
            {
                step === 1?
                <Step1 selectedButton={selectedButton} setSelectedButton={setSelectedButton} />
                : step === 2 ?
                <Step2 selectedButton={selectedButton} />
                : <></>
            }
            <div className="flex justify-between mx-64">
                <button className="flex flex-row items-center justify-between  rounded-3xl p-2 px-5 mt-8 border-2 border-[#50575F]" type="submit"
                onClick={() => {
                    if(step > 0)
                        setStep(step - 1)
                }}>
